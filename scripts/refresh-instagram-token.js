...
// Instagram長期アクセストークンのリフレッシュツール
//
// 使い方:
//   META_APP_ID=xxx META_APP_SECRET=yyy INSTAGRAM_ACCESS_TOKEN=zzz node scripts/refresh-instagram-token.js
//   META_APP_ID=xxx META_APP_SECRET=yyy INSTAGRAM_ACCESS_TOKEN=zzz node scripts/refresh-instagram-token.js --show-token
//
...
if (!META_APP_ID || !META_APP_SECRET || !CURRENT_TOKEN) {
  ...
  console.error('使い方:');
  console.error('  META_APP_ID=xxx META_APP_SECRET=yyy INSTAGRAM_ACCESS_TOKEN=zzz node scripts/refresh-instagram-token.js');
  process.exit(1);
}
...
async function refreshToken() {
  console.log('Instagram長期トークンをリフレッシュ中...\n');

  // POST + bodyで機密情報を送信（URLクエリに含めない）
  ...
  if (showFullToken) {
    ...
    console.log('  META_APP_ID=xxx META_APP_SECRET=yyy INSTAGRAM_ACCESS_TOKEN=zzz node scripts/refresh-instagram-token.js --show-token');
  }

  console.log('');
  console.log('次の手順:');
  console.log('1. Vercelダッシュボード → Settings → Environment Variables');
  console.log('2. INSTAGRAM_ACCESS_TOKEN を新しいトークンに更新');

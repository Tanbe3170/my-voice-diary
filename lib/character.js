...
import { getStyle, getStyleConflicts } from './image-styles.js';
import { PROMPT_WARN_THRESHOLD, IMAGE_PROMPT_HARD_LIMIT } from './prompt-policy.js';
...
function detectStyleConflict(prompt, styleId) {
  const conflicts = getStyleConflicts(styleId);
  ...
  const found = conflicts.filter(keyword => {
    ...
    while (fromIndex < lower.length) {
      ...
      if (idx === -1) break;
      // "NOT <keyword>"パターンは否定文脈のため除外
      const before = lower.slice(Math.max(0, idx - 4), idx).trimEnd();
      if (!before.endsWith('not')) return true;
...
export function composeImagePrompt(diaryImagePrompt, character, styleId) {
  ...
  // 経路非依存のクランプ（character有無に関わらず適用）
  // [Codex archレビュー反復10] 非文字列入力の正規化（null/undefined/数値等）
  ...
  // Scene→Art style→basePrompt→eyeDescriptorの順で合成
  // [Codex archレビュー反復11] Art styleをcharacter要素の前に配置し、
  // DALL-E先頭1000文字内にstyle識別語が残る設計
  const composed = [
    `Scene: ${clampedScene}`,

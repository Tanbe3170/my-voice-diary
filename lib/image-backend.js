...
import { DALLE_MAX_PROMPT } from './prompt-policy.js';
...
async function generateWithDalle(prompt, apiKey, timeoutMs) {
  if (prompt.length > DALLE_MAX_PROMPT) {
    console.warn(`[DALLE_TRUNCATE] DALL-E 3プロンプト切り詰め: ${prompt.length} → ${DALLE_MAX_PROMPT}文字`);
...
export async function generateImageWithFallback(prompt, negativePrompt, deadline) {
  ...
  // タイムアウト系エラーはそのまま再throw（504マッピング用）
